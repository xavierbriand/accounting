...
- **Mystery Guest / Resource Optimism residual (R29, extends R7).** For `tests/unit/core/**` tests not caught by `local/no-mystery-guest-db` (that rule only catches a literal `better-sqlite3` import) — does the test read/write real files or reach other real external state as part of exercising business logic, rather than as a Core-purity self-check reading its own `.ts` source as text (that pattern is legitimate — see `tests/unit/core/events/domain-event.test.ts`)? For `tests/integration/**`/`tests/features/**` tests (where real SQLite/FS is intentional per the testing-tiers table) — does the test set up/verify the external resource it depends on, or silently assume it exists (Resource Optimism)? Report unverified real-resource dependence as P1 findings tagged R29.
...
- **Eager Test / Lazy Test (R29, not lint-covered — human judgment).** Does a single new/modified test invoke several unrelated production methods (Eager Test — harder to tell what's actually under test, breaks the "one behaviour per test" implication of the Unit tier's AAA-pattern rule in `docs/engineering-standards.md`)? Do multiple new/modified tests across the diff call the same production method in ways that could drift or duplicate coverage inconsistently (Lazy Test)? Flag as P3 (soft) unless clearly excessive.
