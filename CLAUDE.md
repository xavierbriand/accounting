...
| **Reduced** | Infra-only (`src/infra`/`src/cli`), root-level build/test/lint config (e.g. `vitest.config.ts`, `eslint.config.js`, `tsconfig*.json`), behavior-changing `harness/` code, or any `.claude/agents`, `.claude/commands`, or skill spec | Skipped | `sibling-overlap` (plan-reviewer dropped) | `code-reviewer` + `sibling-overlap` | R13 (or R14 adapter) | `docs/plans/story-<id>.md` |
...
5. **Retrospective.** Keep/Change/Try at `docs/retrospectives/story-<id>.md`, with a matching `docs/status.d/<date>-story-<id>.md` fragment (format: `docs/status.d/README.md`) committed in the same retro commit — never a separate follow-up. New rules add a row to § 8. Try items are valid only as a same-PR edit (cite the file) or a filed issue (cite `#N`) — dod-check's advisory `try-unfunneled` finding checks this (story-h13); "next process-touching PR" is not a valid deferral. *Exit:* both files committed. Merge user-gated.
...
| R34 | § 6 lane table's Reduced-lane trigger includes root-level build/test/lint config files (e.g. `vitest.config.ts`, `eslint.config.js`, `tsconfig*.json`), not just `src/infra`/`src/cli` — closes a wording gap independently rediscovered by story-maint-21, story-maint-22, story-maint-27, story-maint-38, and story-maint-39 (partially addressed by #200, which stopped at listing R15 as an Envelope value) | [story-maint-40](docs/retrospectives/story-maint-40.md) |
| R35 | R16's "optional 4th body slice" also applies when a single file's change is split into multiple commits by distinct concern (e.g. story-maint-38's infra-floor vs. cli-floor split), not only when the change spans process **and** docs (story-maint-34's original carve-out case) | [story-maint-40](docs/retrospectives/story-maint-40.md) |
