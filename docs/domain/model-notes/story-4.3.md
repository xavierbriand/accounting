...
- `FollowThrough` (part 2) — **value object**: `{ perPartner?: ReadonlyMap<partner, { suggested: Money, actual: Money, delta: Money }>, totalSuggested, totalActual, totalDelta, attribution: 'per-partner' | 'totals-only' }`. Compares this month's suggestion against last month's **Contributions** (actual ledger credits). When any contribution in the window cannot be attributed to a partner, the whole section drops to `totals-only` — honest totals rather than wrong detail.
  *Refinement (Phase 4, 2026-07-08, user decision):* the totals-only fallback is **dropped** — it was unreachable from the documented config, which requires every settlement account to name a partner (roster-checked). `FollowThrough` is always per-partner: `attribution` and `ContributionsInWindow.unattributed` removed; `perPartner` required. Invariant 7 is superseded (every Contribution is partner-attributed by construction); invariant 8 stands as "totalActual equals the ledger's net credit sum on the settlement accounts". An untagged-transfers need, if it ever materializes, gets its own story ([#208](https://github.com/xavierbriand/accounting/issues/208) carries the 4.3b residue; the correction-view story is [#202](https://github.com/xavierbriand/accounting/issues/202)).
