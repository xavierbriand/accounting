...
5. **R-tag staleness scan.** Extend `harness/drift-scan/drift-scan.ts` (Module 1) to flag rules that have had zero retro touches in N stories. The R-table currently crystallizes past failures; this would surface rules that may have outlived their failure mode (Principle 7 of [harness-engineering.md Part B](harness-engineering.md#principles-memorize-the-rest-follows)). Small, cheap, sympathetic to the talk's *"verification is the work"* framing. — **Shipped in substance as story-h13's rule-expiry walk** (last-real-exercise greps + disposition rates retired six rules); drift-scan Check G now expires stamped pending markers on a clock, making the staleness pressure standing rather than episodic.
...
- **Rule-from-retro provenance** ([the R-tag table with originating retros](../../CLAUDE.md)) — a reproducible mechanism for *evolving the gate*. The talk says "verification is expensive" but doesn't say how the verification rules themselves should evolve. We have an answer — and since the 2026-07 walk retired six rules on evidence, a demonstrated one in both directions.
...
- **Failure-signature taxonomy** ([Part B](harness-engineering.md#failure-signatures-to-learn-to-spot): lying-summary, helpful-but-wrong, coincidence-pass, safeguard-removal, shim-creep, over-eager-generalization, plan-execution-drift, cargo-cult-guard — plus the four coordinator-level signatures added 2026-07) — vocabulary for naming what goes wrong in agent output *and* in the loop that dispatches the agents. Not present in the talk.
