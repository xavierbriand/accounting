...
| Idempotency firewall: correction rows must not acquire ingest hashes. | `saveCorrection` takes no hash and never touches the hash table (story-4.0 ACL boundary); covered by the round-trip test asserting no hash row is written. |
