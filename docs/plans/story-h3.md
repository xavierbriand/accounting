# Story h3: Harness Module 2 — Claude Code primitives, by niche

> **Branch/story-id note:** originally planned and committed as `story-h2`. Renamed to `story-h3` after discovering `story-h2` was already taken by the merged drift-scan hard-exit cleanup ([PR #125](https://github.com/xavierbriand/accounting/pull/125), closes [#120](https://github.com/xavierbriand/accounting/issues/120)) — a story-id collision that wasn't caught before this branch was opened. The original PR (#130) closed as a side effect of the branch rename; this work continues under a new PR. Quoted commit messages and Phase-2 review findings below retain their original `[story-h2]` trailers/paths since those commits and that review genuinely happened under the old id — only this document's own identity (title, self-references, file paths going forward) uses `story-h3`.
...
- **Sibling work:** 5 open dependabot PRs (#123, #126, #127, #128, #129) — all patch/minor, no harness overlap. 25 open issues; none conflict with story-h3 scope.
- **Working tree:** clean, `main` up to date with `origin/main`.
- **story-h3 branch:** does not exist yet.
...
| `docs/plans/story-h3.md` | Plan | R1 |
| `docs/status.d/2026-05-13-story-h3.md` | Status fragment | R17 |
...
```
Given this plan exists at docs/plans/story-h3.md
When Phase 2 runs
...
No sibling overlap detected. All 5 open PRs are Dependabot patch/minor bumps. 25 open issues include harness curriculum modules #94–#100 but none conflict with story-h3's specific primitive implementations.
...
This produces output like: `story-h3 · lint: pass · h3`

**Config location confirmed (story-h3 implementation):** user-level `~/.claude/settings.json`. Project-level `.claude/settings.json` does not support `statusCommand`. The statusline is per-developer, not per-repo — this is expected and acceptable for a harness primitive. Future harness stories that touch statusline should document this distinction upfront.
