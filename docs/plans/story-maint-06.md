...
| P3 | `@eslint/js` is imported directly in [eslint.config.js](eslint.config.js) but is a transitive dep, not a direct devDep — fragile under hoisting changes. Pin as direct devDep? | rejected | Probe confirms hoisting works on v10.0.1's `eslint` peerDeps. Adding a direct pin is a defensive cleanup, not a v10-blocker. Revisit only if a future change breaks the import — that would be a new bug, not a deferred suggestion. |
