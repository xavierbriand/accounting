...
| --- | --- | --- | --- | --- |
| P1-1 | **adopt** (Phase-4 correction; originally mis-rejected) | suggestion | Acceptance Gherkin scenario should carry a `fails if` annotation. | **Adopt.** Phase-2 originally rejected this on the false premise that `tests/features/*.feature` carries no scenario-level `fails if` annotations; in fact several existing scenarios (e.g. `tests/features/ingest.feature` lines 11, 23, 33, 46, 56, 67) do. Sonnet correctly authored a `fails if` block on the new scenario despite the rejected ruling, and Phase-4 code review surfaced the inconsistency. The suggestion log entry is updated here for record integrity; retrospective adds a Change item ("walk every prose reference / convention precedent before rejecting on `no precedent` grounds"). |
