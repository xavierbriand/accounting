...
- **The plan template nudges spec/process stories into a dod-check trap; document the prose-acceptance escape.** The template's `## Gherkin acceptance scenarios` section invites a ` ```gherkin ` `Scenario:` block — but dod-check's Gherkin↔step gate (h6) hard-fails any plan-declared scenario with no backing `tests/features/*.feature` file, and a spec-only story has none. CI caught it (green locally, red on the dod-check step) because the local runs didn't invoke dod-check with the PR env. **Fix applied:** write the acceptance as prose Given/When/Then + `fails if`, no `gherkin` fence. **Try:** either add a one-line note to `docs/templates/plan-template.md` ("spec/process stories with no feature file: write acceptance as prose, not a `gherkin` `Scenario:` block") or teach dod-check a plan-only marker — so the next spec story doesn't rediscover this at CI. Also worth wiring dod-check into the local pre-push sanity gate, not just CI.
...
| Plan-template prose-acceptance note (or dod-check plan-only marker) + dod-check in local pre-push gate | future harness story (candidate: h10/h11) | open |
