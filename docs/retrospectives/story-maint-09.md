...
## Post-hoc disposition (story-maint-12)

- **Try-1 (composition-root empty-collection grep at planning time): subsumed by Try-2 (R4 — composition-root subprocess test rule).** Rationale: R4 catches the bug class structurally — the subprocess test running against a real fixture surfaces empty-collection construction immediately. A separate planning-time grep would be belt-and-braces with diminishing returns: the grep catches a narrow pattern (`new X([], …)`); R4 catches *any* wiring bug, including factories not threaded through, swapped Infra adapters, etc. Without a curated list of anti-patterns, "grep the composition root for X" decays into "vibe-check before planning" — rule sprawl, low signal. Decision recorded: don't codify Try-1; rely on R4.
